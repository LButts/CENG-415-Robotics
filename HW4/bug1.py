#!/usr/bin/env python3
import time
import rclpy
import sys
from rclpy.node import Node

from time import *
from std_msgs.msg import Float32
from std_msgs.msg import ByteMultiArray
from geometry_msgs.msg import Pose2D
from sensor_msgs.msg import LaserScan
from veranda.SimTimer import SimTimer

from math import *
from struct import *

# Globals
x, y, theta = 0.0, 0.0, 0.0 # gps loc of robot
goalX = 7
goalY = -8
dtheta = 0.0

q_min = sys.float_info.max
q_start_x, q_start_y = 0.0, 0.0
is_boundary_following = False
goalDist = sys.float_info.max

# Lidar Variables:
left = sys.float_info.max
mid_left = sys.float_info.max
right = sys.float_info.max
mid_right = sys.float_info.max
mid = sys.float_info.max

r = 0.5 # radius of wheel (m)
l = 0.6 # half the distance between wheels (m)
rps = 0.5

# Veranda Sensors
##################################################################

# GPS
def get_position(message):
	global x, y, goalDist, theta, dtheta
	#print("Robot is at (" + str(message.x) + "," + str(message.y) + ") facing " + str((message.theta*180/pi) % 360) + " degrees")
	#print("----------------")
	x = message.x
	y = message.y
	goalDist = sqrt((goalX - x)*(goalX - x) + (goalY - y)*(goalY-y))
	theta = message.theta + pi / 2.0
	dtheta_calc() # dtheta updated

# Lidar
def get_lidar(message):
	global mid, left, right, q_start_x, q_start_y
	if (is_boundary_following):
		q_start_x = x
		q_start_y = y
		left = message.ranges[49]
		left = message.ranges[36]
		mid = message.ranges[24]
		right = message.ranges[12]
		right = message.ranges[0]

		print ("Left: " + str(left))
		print ("Right: " + str(right))
		print ("Mid: " + str(mid))


# Touch Sensor
def get_hit(message):
	global timer_handle
	simTime.destroy_timer(timer_handle)

	print("_________________________")
	print("I'm Hit I'm Hit I'm Hit")
	print("_________________________")

	timer_handle = simTime.create_timer(0.15, back_up)


##################################################################
# Callbacks:
##################################################################

def start():
	global timer_handle
	simTime.destroy_timer(timer_handle)
	timer_handle = simTime.create_timer(0.1, face_goal)


def move_to_goal():
	global timer_handle
	simTime.destroy_timer(timer_handle)

	msgl = Float32() # left wheel msg
	msgr = Float32() # right wheel msg

	if ( reach_goal() ) :

		print("********************************************")
		print("MISSION COMPLETE!!!")
		print("MISSION COMPLETE!!!")
		print("MISSION COMPLETE!!!")
		print("********************************************")
		msgl.data = 0.0
		msgr.data = 0.0
		publeft.publish(msgl)
		pubright.publish(msgr)

		node.destroy_node()
		rclpy.shutdown()


	print("_________________________")
	print("move_to_goal")
	print("_________________________")

	t = 0.1
	msgl.data = 3.0
	msgr.data = 3.0
	publeft.publish(msgl)
	pubright.publish(msgr)

	timer_handle = simTime.create_timer(0.1, move_to_goal)

def back_up() :
	global publeft, pubright, timer_handle, is_boundary_following
	simTime.destroy_timer(timer_handle)

	print("___________________________")
	print("back_up")
	print("___________________________")

	msgl = Float32() # left wheel msg
	msgr = Float32() # right wheel msg

	is_boundary_following = True

	t = 0.1
	msgl.data = -2.0
	msgr.data = -2.0
	publeft.publish(msgl)
	pubright.publish(msgr)

	timer_handle = simTime.create_timer(t, turn_right)

#def turn_type() :


def turn_right() :
	global publeft, pubright, timer_handle
	simTime.destroy_timer(timer_handle)

	msgl = Float32() # left wheel msg
	msgr = Float32() # right wheel msg

	print("_________________________")
	print("turn_right")
	print("_________________________")

	# turn 90
	rps = 0.5 # phidot1 = -phidot2
	t = (pi / 2) * l / (r*rps) # pivot time
	msgl.data = rps
	msgr.data = -rps
	publeft.publish(msgl)
	pubright.publish(msgr)

	timer_handle = simTime.create_timer(t, motion)

def motion():
	global publeft, pubright, timer_handle
	simTime.destroy_timer(timer_handle)

	msgl = Float32() # left wheel msg
	msgr = Float32() # right wheel msg

	t = 0.1

	if (left > 1.25) :
		print("_________________________")
		print("Slight LEFT, left = " + str(left))
		print("_________________________")
		msgl.data = 0.5
		msgr.data = 1.5 # Test this value
		publeft.publish(msgl)
		pubright.publish(msgr)
		timer_handle = simTime.create_timer(t, motion)

	elif (left < 1.2) :
		print("_________________________")
		print("Slight RIGHT")
		print("_________________________")
		msgl.data = 1.5 # Test this value
		msgr.data = 0.5
		publeft.publish(msgl)
		pubright.publish(msgr)
		timer_handle = simTime.create_timer(t, motion)
	else :
		print("_________________________")
		print("Move forward a LITTLE")
		print("_________________________")

		msgl.data = 2.0
		msgr.data = 2.0
		publeft.publish(msgl)
		pubright.publish(msgr)
		timer_handle = simTime.create_timer(t, motion)

def face_goal():
	global publeft, pubright, rps, timer_handle
	simTime.destroy_timer(timer_handle)

	msgl = Float32() # left wheel msg
	msgr = Float32() # right wheel msg

	is_boundary_following = False

	print("_________________________")
	print("face_goal")
	print("_________________________")

	rps = 0.5
	print("dtheta: " + str(dtheta))
	t = dtheta * l / (r*rps) # pivot time to face goal
	print("t = " + str(t))
	msgl.data = -rps
	msgr.data = rps
	publeft.publish(msgl)
	pubright.publish(msgr)

	timer_handle = simTime.create_timer(t, move_to_goal)


##################################################################
# Functions:
##################################################################

def dtheta_calc():
	global dtheta, theta

	if theta < 0 :
		theta += 2.0 * pi
	if theta > 2.0 * pi :
		theta -= 2.0 * pi

	dtheta = atan2((goalY - y), (goalX - x))

	if dtheta < 0 :
		dtheta += 2.0 * pi
	if dtheta > 2.0 * pi :
		dtheta -= 2.0 * pi

	dtheta = dtheta - theta

	if dtheta < 0 :
		dtheta += 2.0 * pi
	if dtheta > 2.0 * pi :
		dtheta -= 2.0 * pi

def reach_goal() :
	if( (x < goalX + 3) and (x > goalX - 2) and
		(y < goalY + 2) and (y > goalY - 2) ) :
		return True
	return False

##################################################################
# Setup Robot & Sensors
rclpy.init()
node = Node("BM_node")
simTime = SimTimer(True, 'veranda/timestamp', node)
publeft = node.create_publisher(Float32, 'robot0/left_wheel')
pubright = node.create_publisher(Float32, 'robot0/right_wheel')
gps = node.create_subscription(Pose2D, 'robot0/gps', get_position)
lidar = node.create_subscription(LaserScan, 'robot0/lidar', get_lidar)
subtouches = node.create_subscription(ByteMultiArray, 'robot0/touches', get_hit)
##################################################################
# Main Code:
##################################################################

timer_handle = simTime.create_timer(0.25, start)

rclpy.spin(node)

# Clean up and End Program
node.destroy_node()
rclpy.shutdown()
